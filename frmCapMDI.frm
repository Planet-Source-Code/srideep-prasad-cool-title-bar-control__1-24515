...
Dim ByPassRefresh As Boolean, Init As Boolean, MaxDim As Long
Dim pLeft As Long, pTop As Long
Dim pVisible As Boolean
Dim WithEvents CurMDIChild As Form
Attribute CurMDIChild.VB_VarHelpID = -1
...
Sub SetParent(Frm As Form)
    ...
    AttachMessage Me, Pfrm.hwnd, WM_NCPAINT
    AttachMessage Me, Pfrm.hwnd, WM_WINDOWPOSCHANGED

    Set ParentFrm = Frm
    
    Set WC = New clsStyleMDI
    Set ParentForm = Frm
    PHwnd = Pfrm.hwnd
...
Private Sub CurMDIChild_Resize()
    Cap.Caption = Pfrm.Caption
End Sub

Private Sub CurMDIChild_Unload(Cancel As Integer)
    Cap.Caption = Pfrm.Caption
End Sub
...
Sub RefreshAll()
    If ByPassRefresh = True Then Exit Sub
    If Pfrm.ActiveForm Is Nothing Then
    'DO nothing
    Else
        Set CurMDIChild = Pfrm.ActiveForm
    End If
    ByPassRefresh = True
...
'   End If
    ...
    If Me.Visible = True Then
        If Pfrm.Visible = False Then Me.Visible = False
    Else
        If Pfrm.Visible = True Then Me.Visible = True
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    DetachMessage Me, Pfrm.hwnd, WM_NCPAINT
...
'               If ModalFlag = False Then Title.SetTopMost False
    ...
    If hwnd = Pfrm.hwnd Then
        ...
        If iMsg = WM_SIZE Then Cap.Caption = Pfrm.Caption
        ...
        If iMsg = WM_NCPAINT Or iMsg = WM_PAINT Then
            If Cap.Caption <> Pfrm.Caption Then Cap.Caption = Pfrm.Caption
        End If
...
If GetMenuItemCount(GetMenu(Pfrm.hwnd)) <> -1 Then
    If GetMenuItemCount(GetMenu(Pfrm.hwnd)) <> 0 Then
        ChkMenu = True
    End If
End If
'If ScaleY(Pfrm.ScaleHeight, vbTwips, vbPixels) + GetSystemMetrics(SM_CYMENU) + sBorder = Screen.Height / Screen.TwipsPerPixelY Then ChkMenu = True
End Function
...
'   SetWindowPos Me.hwnd, HWND_BOTTOM, 0, 0, 0, 0, SWP_NOSIZE Or SWP_NOACTIVATE Or SWP_NOMOVE
'   Title.SetTopMost False
